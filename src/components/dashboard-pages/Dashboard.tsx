...
export default function Dashboard() {
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="flex h-full font-sans text-[#121415] selection:bg-[#8A2532] selection:text-white">
        ...
        <header className="bg-[#F5F5F4]/90 backdrop-blur-md border-b border-[#DCDCDA] px-6 md:px-10 py-4 md:py-0 h-auto md:h-20 shrink-0 sticky top-0 z-20 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-[#121415] tracking-tight">{t("extra.t118")}</h1>
            <p className="text-sm text-[#4A4E51] font-medium mt-0.5">{t("extra.t190")}</p>
          </div>
          ...
          <div className="px-6 md:px-10 pb-6 grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
            ...
            <div className="bg-white p-5 rounded-2xl border border-[#DCDCDA] shadow-sm flex flex-col justify-between min-w-0">
              <div className="flex items-center gap-2 text-[#4A4E51] mb-2 shrink-0">
                ...
                <span className="text-xs font-medium uppercase tracking-wider">{t("extra.t224")}</span>
              </div>
            ...
            <div className="bg-[#FFF4F4] p-5 rounded-2xl border border-[#FDE8E8] shadow-sm flex flex-col justify-between">
              <div className="flex items-center gap-2 text-[#8A2532] mb-2">
                ...
                <span className="text-xs font-medium uppercase tracking-wider">{t("extra.t101")}</span>
              </div>
                  ...
                  >
                    ...
                    {!isBookingsLoading && filteredWaiting.length === 0 && (
                      <div className="flex flex-col items-center justify-center h-32 text-center border-2 border-dashed border-[#DCDCDA] rounded-2xl mx-1 bg-[#F5F5F4]/50">
                        <span className="text-sm font-medium text-[#8B9194]">{t("extra.t239")}</span>
                        <span className="text-[10px] text-[#8B9194]/70 mt-1 uppercase tracking-wider">{t("extra.t174")}</span>
                      </div>
                    )}
                  ...
                  >
                    ...
                    {!isDashboardLoading && filteredInChair.length === 0 && (
                      <div className="flex flex-col items-center justify-center h-32 text-center border-2 border-dashed border-[#4a6b53]/30 rounded-2xl mx-1 bg-white/50">
                        <span className="text-sm font-medium text-[#4a6b53]/70">{t("extra.t201")}</span>
                        <span className="text-[10px] text-[#4a6b53]/50 mt-1 uppercase tracking-wider">{t("extra.t159")}</span>
                      </div>
                    )}
                    {provided.placeholder}
                  </div>
                  ...
                  >
                    ...
                    {!isDashboardLoading && filteredCompleted.length === 0 && (
                      <div className="flex flex-col items-center justify-center h-32 text-center border-2 border-dashed border-[#DCDCDA]/50 rounded-xl mx-1">
                        <span className="text-xs font-medium text-[#8B9194]">{t("extra.t148")}</span>
                      </div>
                    )}
                    {provided.placeholder}
                  </div>
        ...
        >
          ...
          >
            ...
            <div className="p-8 pb-6 shrink-0">
              <h2 className="text-2xl font-semibold text-[#121415] tracking-tight">{t("extra.t162")}</h2>
              <p className="text-sm text-[#4A4E51] font-medium mt-1">{t("extra.t241")}</p>
            </div>
            <form className="px-8 py-2" onSubmit={(e) => { e.preventDefault(); handleAddGuest(); }}>
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-[#121415] mb-2">{t("extra.t164")}</label>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {servicesData.length > 0 ? servicesData.map(s => (
                      ...
                      <span className="text-sm text-[#8B9194]">{t("extra.t304")}</span>
                    )}
                  </div>
                </div>
              </div>
