...
interface Guest {
  ...
  const filteredWaiting = selectedFilter === "All" ? waitingGuests : waitingGuests.filter(g => g.master === selectedFilter);
  const filteredInChair = selectedFilter === "All" ? inChairGuests : inChairGuests.filter(g => g.master === selectedFilter);
  const filteredCompleted = selectedFilter === "All" ? completedGuests : completedGuests.filter(g => g.master === selectedFilter);
                        ...
                        {(provided, snapshot) => (
                          ...
                          >
                            ...
                            <div className="flex justify-between items-start mb-4 pl-2">
                              <div className="flex flex-col">
                                <span className="text-xs font-medium text-[#4a6b53] uppercase tracking-wider mb-1">Master: {guest.master}</span>
                        ...
                        <div className="absolute z-50 w-full mt-2 bg-white border border-[#DCDCDA] rounded-xl shadow-lg py-1 animate-in fade-in slide-in-from-top-2 duration-200">
                          {mastersList.map((master) => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-2.5 text-sm font-medium transition-colors hover:bg-[#F5F5F4] ${masterName === master ? 'text-[#121415] bg-[#F5F5F4]/50' : 'text-[#4A4E51]'}`}
                            >
                              {master}
                            </button>
                          ))}
