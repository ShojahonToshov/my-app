    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] max-w-sm w-full border border-gray-100">
          <h1 className="text-2xl font-bold text-center mb-8 text-[#121415]">{t("extra.t238")}</h1>
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <Button type="submit" className="w-full mt-2" size="lg">{t("extra.t177")}</Button>
          </form>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
          <div>
            <h1 className="text-2xl font-bold text-[#121415]">{t("extra.t227")}</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-[#F8F9FA] text-[#8B9194] border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 font-medium">{t("extra.t298")}</th>
                  <th className="px-6 py-4 font-medium">{t("extra.t208")}</th>
                  <th className="px-6 py-4 font-medium">{t("extra.t299")}</th>
                  <th className="px-6 py-4 font-medium text-right">{t("extra.t332")}</th>
                </tr>
              </thead>
                      ...
                      <td className="px-6 py-4 align-top">
                        <div className="space-y-1.5">
                          <div className="text-[14px]">
                            <span className="text-gray-400 mr-2 inline-block w-12">{t("extra.t135")}</span>
                            <span className="font-semibold text-gray-900">
                              {user.phone ? (user.phone.startsWith('+') ? user.phone : '+' + user.phone) : <span className="italic text-gray-400 font-normal">{t("extra.t105")}</span>}
                            </span>
                          </div>
                          <div className="text-[14px]">
                            <span className="text-gray-400 mr-2 inline-block w-12">{t("extra.t145")}</span>
                            <span className="font-semibold text-gray-900">
                              {user.email ? user.email : <span className="italic text-gray-400 font-normal">{t("extra.t105")}</span>}
                            </span>
                          </div>
                          <div className="text-[11px] text-gray-400 mt-2 font-mono flex items-center">
                            <span className="mr-2">{t("extra.t206")}</span>{user.id}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 align-top">
                        <div className="space-y-1 text-[13px]">
                          {user.profile ? (
                            <>
                              {user.profile.full_name && <div><span className="text-gray-400 mr-2">{t("extra.t195")}</span><span className="font-medium">{user.profile.full_name}</span></div>}
                              {user.profile.role && <div><span className="text-gray-400 mr-2">{t("extra.t179")}</span><span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-md text-[11px] uppercase font-bold tracking-wider">{user.profile.role}</span></div>}
                              {user.business_name && (
                                <div className="mt-2">
                                  <span className="text-gray-400 mr-2">{t("extra.t286")}</span>
                              ...
                              <details className="mt-3 text-xs text-gray-400 cursor-pointer group-open">
                                <summary className="hover:text-gray-600 outline-none">{t("extra.t133")}</summary>
                            ...
                            <span className="text-gray-400 italic">{t("extra.t205")}</span>
                          )}
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 border border-gray-100">
            ...
            <div className="flex gap-3 justify-end">
              <Button variant="outline" onClick={() => setModalConfig({ isOpen: false, mode: 'single' })}>{t("extra.t284")}</Button>
