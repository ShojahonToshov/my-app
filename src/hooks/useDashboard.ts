...
export default function useDashboard() {
  ...
  const [activeStaffFilter, setActiveMasterFilter] = useState("All");
  ...
  const uniqueMasters = useMemo(() => {
    const staff = new Set(tickets.map((t: TicketDTO) => t.staff));
  ...
  const completeMutation = useMutation({
    mutationFn: async ({ currentTicketId, staff }: { currentTicketId: string; staff: string }) => {
      await BookingService.updateBookingStatus(currentTicketId, "completed");
      const data = await BookingService.getBookings();
      const nextMapped = data.find((t: ApiBookingDTO) => t.staffName === staff && t.status === "upcoming" && t.id !== currentTicketId);
    ...
    onMutate: async ({ currentTicketId, staff }: { currentTicketId: string; staff: string }) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.bookings.all });
      const previousTickets = queryClient.getQueryData<TicketDTO[]>(queryKeys.bookings.all);
      
      queryClient.setQueryData<TicketDTO[]>(queryKeys.bookings.all, (old) => {
        if (!old) return [];
        const nextTicket = old.find((t: TicketDTO) => t.status === "waiting" && t.staff === staff);
    ...
    onSuccess: (data, variables) => {
      if (data.nextMapped) {
        toast.success("Visit completed", { description: `${data.nextMapped.customerName || "Guest"} was automatically called to the chair.` });
      } else {
        toast.success("Visit completed", { description: `Queue for ${variables.staff} is now clear.` });
      }
    },
    onError: (err, newTodo, context) => {
      queryClient.setQueryData(queryKeys.bookings.all, context?.previousTickets);
  ...
  const handleCompleteAndNext = useCallback((currentTicketId: string, staff: string) => {
    completeMutation.mutate({ currentTicketId, staff });
  }, [completeMutation]);

  const handleAddDelay = useCallback((staff: string, mins: number) => {
    setMasterDelays((prev: Record<string, number>) => ({
      ...prev,
      [staff]: Math.max(0, (prev[staff] || 0) + mins)
    }));
    if (mins > 0) {
      toast.warning(`Schedule shifted by +${mins} min`, { description: `Clients of ${staff} have been notified.` });
    } else {
      toast.success(`Schedule caught up`, { description: `Delay for ${staff} was reduced.` });
    }
  }, []);
  ...
  const handleAddTicket = (e: FormEvent<HTMLFormElement>, selectedService: string, callback?: () => void) => {
    ...
    const customerName = (formData.get("customerName") as string) || "Guest";
    const selectedMaster = (formData.get("staffName") as string) || uniqueMasters[0] || "Staff";
  ...
  const filteredTickets = useMemo(() => tickets.filter(
    (tkt) => activeStaffFilter === "All" || tkt.staff === activeStaffFilter
  ), [tickets, activeStaffFilter]);
  ...
  return {
    ...
    activeStaffFilter, setActiveMasterFilter,
